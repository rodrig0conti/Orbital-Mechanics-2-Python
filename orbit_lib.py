...
#Funciones de Assignment 2, TLE utilities



def get_hubble_tle_params():
    # Valores aproximados del HST (ejemplo)
...
def get_hubble_epoch():
    # Epoch de ejemplo (puedes poner uno real si quieres)
    return 24073.12345678


def tle_params_from_degrees(i_deg, Omega_deg, e, w_deg, Me_deg, revs_per_day):
    """
    Convierte parámetros TLE en grados a radianes.
    Devuelve: e, revs_per_day, Me(rad), Omega(rad), i(rad), w(rad)
    """
    i = i_deg * DTOR
...
def get_iss_epoch():
    # Epoch real del TLE usado
    return 24073.51041667


def compute_initial_state_from_tle():
    """
    Usa state_from_tle_params para obtener r0, v0 en ECI.
    """
...
def propagate_one_orbit(r0, v0, num_points=500):
    """
    Propaga una órbita completa usando parámetros iniciales r0, v0.
    Devuelve times, r_list, v_list.
    """
    h, e, theta0, Omega, i, w = orbit_params_from_state(r0, v0)
...
def plot_orbit_eci(r_list, r0):
    """
    Grafica la órbita en ECI con la Tierra rotada según el epoch.
    """
    ...
    # Epoch del TLE (puedes parametrizarlo)
    epoch = 24073.51041667

    # Ángulo sideral inicial
