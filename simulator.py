...
class SimCanvas(scene.SceneCanvas):
    ...
    def make_default_satellite(self,satellite_model):
        ...
        if len(self.vertex_cols) > 0:
            self.satellite = Mesh(self.vertices, self.faces, vertex_colors = self.vertex_cols, shading='smooth', color='white',parent=self.view.scene)
        else:
            self.satellite = Mesh(self.vertices, self.faces, shading='smooth', color='white',parent=self.view.scene)
        self.satellite.shading_filter.shininess = 100
        self.satellite.shading_filter.specular_coefficient =0.7 
        self.satellite.shading_filter.diffuse_coefficient = 0.5
        self.satellite.shading_filter.ambient_coefficient = 0.8
        self.satellite.shading_filter.ambient_light = (1,1,1,0.5)
        self.satellite.shading_filter.specular_light = (1,1,1,1.0)
        self.satellite.shading_filter.diffuse_light = (1,1,1,0.4)
        self.satellite.shading_filter.light_dir = self.light_dir[:3]
        s = ol.R_E/(12*self.scale_factor*np.max(np.abs(self.vertices)))
        #s = 1
    ...
    def make_default_frames(self):
        ...
        self.scene_list['body frame'] = (self.body_frame,10.5*self.scene_list['satellite'][1])
...
class Simulator:
  ...
  def sim_runner(self):
    ...
    while not self.sim_close.is_set() and self.t < self.t_e:
      ...
      if self.animate and (self.t-self.t_anim > self.anim_dt*self.speed_factor):
        self.t_anim = self.t
        states = self.scenario.get()
        self.sim_queue.put(states)
        time.sleep(self.anim_dt/2)
      self.t += self.t_step
